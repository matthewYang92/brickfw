...
/**
 * Created by yangtian on 2018/11/24.
 * E-Mail Address: 443275705@qq.com
 */
public class BetterRecyclerView extends BrickRecyclerView {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (getScrollState() != SCROLL_STATE_DRAGGING) {
                    ...
                    if (canScrollHorizontally && Math.abs(dx) > mTouchSlop && (Math.abs(dx)
                        >= Math.abs(dy) || canScrollVertically)) {

                        startScroll = true;
                    }

                    if (canScrollVertically && Math.abs(dy) > mTouchSlop && (Math.abs(dy)
